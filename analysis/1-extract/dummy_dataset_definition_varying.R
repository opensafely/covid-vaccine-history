...
start_day <- as.integer(study_dates$start_date - index_date)
...
known_variables <- c("index_date", "start_date", "index_day", "start_day")
...
sim_list_vax_info <- lst(
  ...
  # covid vax any
  covid_vax_1_day = bn_node( ~ runif(n = ..n, start_day, start_day + 365), missing_rate = ~ 0.05, ),
  ...
  covid_vax_product_2 = bn_node(~ if_else(
    runif(..n) < 0.98, covid_vax_product_1, "az"
  ), needs = "covid_vax_2_day"),
...
sim_list_varying_i <- function(i) {
  ...
  lst(
    ...
    "age_{i}" := bn_node( ~ as.integer(rnorm(
      n = ..n, mean = 60, sd = 14
    )), needs = vax_variable),
    "imd_{i}" := bn_node(
    ...
    "registered_{i}" := bn_node( ~ rbernoulli(n = ..n, p = 0.99), needs = vax_variable),
    "stp_{i}" := bn_node( ~ factor(as.integer(
      runif(n = ..n, 1, 36)
    ), levels = 1:36), needs = vax_variable),
    "region_{i}" := bn_node(
    ...
    # PRIMIS
    "crd_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # chronic respiratory disease
    "chd_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # chronic heart disease
    "ckd_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # chronic kidney disease
    "cld_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # chronic liver disease
    "cns_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # chronic neurological disease
    "learndis_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # learning Disability
    "diabetes_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # diabetes
    "immunosuppressed_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # immunosuppress grouped
    "asplenia_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # asplenia or Dysfunction of the Spleen
    "severe_obesity_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # immunosuppress grouped
    "smi_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    # severe mental illness
    "primis_atrisk_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    ...
    "creatinine_age_{i}" := bn_node(~ as.integer(rnorm(
      n = ..n, mean = 60, sd = 14
    )), needs = vax_variable),
    "copd_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    "down_sydrome_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
    "sickle_cell_{i}" := bn_node(~ rbernoulli(n = ..n, p = 0.02), needs = vax_variable),
  )
}
...
sim_list <- splice(sim_list_vax_info, sim_list_varying)
...
dummydata_processed <- dummydata %>%
  ...
  mutate(across(
    starts_with("covid_vax_product_"),
    ~ factor(
      .,
      levels = names(vax_product_lookup),
