...
summary_validation <-
  ...
  summarise(
    ...
    n_earlier_than_start_date = round_any(sum(vax_date < start_date, na.rm = TRUE), sdc_threshold),
    pct_earlier_than_start_date = n_earlier_than_start_date / n,
    n_earlier_than_firstpossiblevax_date = round_any(sum(vax_date < firstpossiblevax_date, na.rm = TRUE), sdc_threshold),
